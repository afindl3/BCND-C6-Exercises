...
contract('ExerciseC6A', async (accounts) => {
  ...
  it('contract owner can register new user', async () => {
    // ARRANGE
    let caller = accounts[0]; // This should be config.owner or accounts[0] for registering a new user (first ganache account)
  ...
  it('function call is made when multi-party threshold is reached', async () => {
    // ARRANGE
    let admin1 = accounts[1]; // Need to use accounts and not testAddresses if doing write transactions
